...
When a managed file is modified directly (e.g. manual `CMakeLists.txt` or `vcpkg.json` edits, etc.), you update `.wiswa.jsonnet` so that running Wiswa again would produce the same
result.
...
2. **Read the changed managed files** (e.g. `CMakeLists.txt`, `vcpkg.json`, `package.json`) to identify what changed.
